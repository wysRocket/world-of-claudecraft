...
describe('cssEffectsTier - auto detection', () => {
  it('desktop Chromium on a healthy GPU keeps full effects', () => {
    expect(
      cssEffectsTier({ engine: 'chromium', version: 124, mobile: false, renderTier: 'high' }),
    ).toBe('full');
  });
  it('any mobile drops at least to reduced; webkit/gecko mobile go minimal', () => {
    expect(
      cssEffectsTier({ engine: 'chromium', version: 124, mobile: true, renderTier: 'high' }),
    ).toBe('reduced');
    expect(
      cssEffectsTier({ engine: 'webkit', version: 17, mobile: true, renderTier: 'high' }),
    ).toBe('minimal');
    expect(
      cssEffectsTier({ engine: 'gecko', version: 126, mobile: true, renderTier: 'high' }),
    ).toBe('minimal');
  });
  it('the renderer dropping to the low GPU tier pulls the DOM down with it', () => {
    expect(
      cssEffectsTier({ engine: 'chromium', version: 124, mobile: false, renderTier: 'low' }),
    ).toBe('reduced');
    expect(
      cssEffectsTier({ engine: 'chromium', version: 124, mobile: true, renderTier: 'low' }),
    ).toBe('minimal');
  });
  it('old Safari / old Firefox get reduced on desktop; modern stay full', () => {
    expect(
      cssEffectsTier({ engine: 'webkit', version: 14, mobile: false, renderTier: 'high' }),
    ).toBe('reduced');
    expect(
      cssEffectsTier({ engine: 'webkit', version: 17, mobile: false, renderTier: 'high' }),
    ).toBe('full');
    expect(
      cssEffectsTier({ engine: 'gecko', version: 102, mobile: false, renderTier: 'high' }),
    ).toBe('reduced');
    expect(
      cssEffectsTier({ engine: 'gecko', version: 126, mobile: false, renderTier: 'high' }),
    ).toBe('full');
  });
  it('desktop Safari on the medium tier trims to reduced (backdrop-filter cost)', () => {
    expect(
      cssEffectsTier({ engine: 'webkit', version: 17, mobile: false, renderTier: 'medium' }),
    ).toBe('reduced');
  });
  it('unknown engine stays conservative', () => {
    expect(
      cssEffectsTier({ engine: 'unknown', version: 0, mobile: false, renderTier: 'high' }),
    ).toBe('reduced');
    expect(
      cssEffectsTier({ engine: 'unknown', version: 0, mobile: true, renderTier: 'high' }),
    ).toBe('minimal');
  });
});
...
describe('browserBodyClasses', () => {
  it('emits engine / device / tier classes', () => {
    ...
    expect(browserBodyClasses(env, 'minimal')).toEqual([
      'engine-webkit',
      'is-mobile',
      'fx-minimal',
    ]);
