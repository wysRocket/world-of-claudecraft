...
const result = await page.evaluate(async () => {
  ...
  mob.pos.z = p.pos.z;
  mob.pos.y = p.pos.y;
  const intBefore = p.stats.int,
    maxManaBefore = p.maxResource;
  ...
  return {
    ok: cursed,
    intBefore,
    intAfter: p.stats.int,
    maxManaBefore,
    maxManaAfter: p.maxResource,
...
if (!result.ok) {
  ...
  await page.evaluate(() => {
    ...
    let mob = null,
      best = 1e9;
    for (const e of sim.entities.values()) {
      if (e.templateId === 'wyrmcult_zealot' && !e.dead) {
        mob = e;
        break;
      }
    }
