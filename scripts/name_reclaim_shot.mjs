...
async function waitForServer() {
  for (let i = 0; i < 120; i++) {
    try {
      if ((await fetch(`${BASE}/api/realms`)).ok) return;
    } catch {
      /* not up yet */
    }
    await sleep(500);
...
try {
  ...
  const bBlocked = await api('/api/characters', {
    method: 'POST',
    token: tokenB,
    body: { name: NAME, class: 'mage', skin: 0 },
  });
  log(`B create while A live  -> ${bBlocked.status} ${JSON.stringify(bBlocked.data)}`);
  if (bBlocked.status !== 409)
    throw new Error('expected 409 while A is live, got ' + bBlocked.status);

  // 3. A deactivates -> the old account becomes "invalid".
  const deact = await api('/api/account/deactivate', {
    method: 'POST',
    token: tokenA,
  ...
  const bReclaim = await api('/api/characters', {
    method: 'POST',
    token: tokenB,
    body: { name: NAME, class: 'mage', skin: 0 },
  });
  ...
  const react = spawnSync(
    ...
    [
      ...
      `un=${userA}`,
      '-c',
      `UPDATE accounts SET deactivated_at = NULL WHERE username = :'un';`,
    ],
    { encoding: 'utf8' },
  );
  ...
  log(
    'captured old-account char-select (force-rename prompt):',
    renameRow ? 'rename-required row present' : 'NO rename row',
  );
  await pageA.close();
